...
mysql --user=root --password=$MYSQL_ROOT_PASSWORD -e "CREATE DATABASE $YAMBA_QUEUESADILLA_DATABASE";

for file in /docker-entrypoint-initdb.d/schema/*.sql; do
    echo $file;
    mysql --user=root --password=$MYSQL_ROOT_PASSWORD < $file $([[ $(echo $file | grep -v "_vendor.sql$") ]] && echo $MYSQL_DATABASE || echo $YAMBA_QUEUESADILLA_DATABASE);
done
